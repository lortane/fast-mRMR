 ...
 */
bool contains(std::string key, std::map<std::string, byte> mymap)
{
    std::map<std::string, byte>::iterator it = mymap.find(key);
    if (it == mymap.end()) {
        return false;
    }
    return true;
...
int main(int argc, char* argv[])
{
    ...
    std::string token;
    byte data;
    std::vector<byte> lastCategory;  // Cuantas categorias tiene cada feature.
    std::vector<std::map<std::string, byte> > translationsVector;  // Vector de mapas de traduccion
    std::map<std::string, byte> translations;  // mapa de traduccion string->byte ara cada feature
    ...
    std::ofstream outputFile("data.mrmr", std::ios::out | std::ios::binary);
        ...
        while (getline(inputFile, line)) {
            ...
            while (strtk.hasMoreTokens()) {
                ...
                if (!contains(token, translationsVector[featurePos])) {
                    translationsVector[featurePos][token] = lastCategory[featurePos];
